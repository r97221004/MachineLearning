...
# The train_test_split function already shuffles the training sets internally before splitting.
...
# Most algorithms in scilitlearn already support multiclass classification by default via the one-versus-Rest (OvR)
...
get_ipython().run_line_magic('pinfo', 'Perceptron')
...
lr.predict(X_test_std[0, :].reshape(1, -1)) # Even for a single sampe, scikit-learn expects a two-dimensional array data input.
...
get_ipython().run_line_magic('pinfo', 'np.logical_xor')
...
np.logical_xor(X_xor[:, 0] > 0, X_xor[:, 1] > 0)[:5]
...
get_ipython().run_line_magic('pinfo', 'export_graphviz')
...
Image(filename='tree_.png', width=600) 
...
get_ipython().system(' python ../.convert_notebook_to_script.py --input ch03.ipynb --output ch03.py')
